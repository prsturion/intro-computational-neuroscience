...
def mh_inf(Vd: ArrayLike) -> ArrayLike:
    """
    m_h,∞(Vd) = 1 / (1 + exp[166,667 * (Vd + 0.070)]).
    This function calculates the steady-state value of m_h for the voltage-gated ion channels.

    Parameters:
    Vd : ArrayLike
        Voltage (in mV) at which the m_h gating variable is calculated.

    Returns:
    ArrayLike
        Steady-state value of m_h,∞.
    """
    return 1.0 / (1.0 + np.exp(166.667 * (Vd + 0.070)))

def tau_mh(Vd: ArrayLike) -> ArrayLike:
    """
    τ_mh(Vd) = 0.272 + 1.499 / (1 + exp[-114,548 * (Vd + 0.0422)]).
    This function calculates the time constant τ_mh for the voltage-gated m_h.

    Parameters:
    Vd : ArrayLike
        Voltage (in mV) at which the time constant τ_mh is calculated.

    Returns:
    ArrayLike
        Time constant τ_mh.
    """
    return 0.272 + 1.499 / (1.0 + np.exp(-114.548 * (Vd + 0.0422)))
...
def rhs_dendritic(t: float, y: Sequence[float], params: Dict[str, float]) -> ArrayLike:
    ...
    """
    Vs, Vd, Ca, *_ = y
    mca, mkca, mkahp, mh = y[6], y[7], y[8], y[9]
    p = params
    ...
    dVddt = (-IL - ICa - IKCa * chi_val - IKahp + Ic + Iinj_D - Ih) / p["C_D"]
    return dVddt
...
def rhs_gates(t: float, y: Sequence[float], params: Dict[str, float]) -> tuple[ArrayLike, ...]:
    ...
    m, h, n, mca, mkca, mkahp, mh = y[3], y[4], y[5], y[6], y[7], y[8], y[9]
    ...
    dmh = (mh_inf(Vd) - mh) / tau_mh(Vd)

    return dm, dh, dn, dmca, dmkca, dmkahp, dmh
...
def rhs_full(t: float, y: Sequence[float], params: Dict[str, float]) -> np.ndarray:
    ...
    dm, dh, dn, dmca, dmkca, dmkahp, dmh = rhs_gates(t, y, params)
    return np.array([dVs, dVd, dCa, dm, dh, dn, dmca, dmkca, dmkahp, dmh], dtype=float)
